...
publish-image:
	aws ecr describe-repositories --registry-id=948063967832 --region=us-east-1 --repository-names $(appName) >/dev/null 2>&1 || \
	  aws ecr create-repository --region=us-east-1 --repository-name $(appName)
	`aws ecr get-login --registry-id 948063967832 --region us-east-1 --no-include-email`
	docker tag $(appName):$(rev) 948063967832.dkr.ecr.us-east-1.amazonaws.com/$(appName):$(rev)
	docker push 948063967832.dkr.ecr.us-east-1.amazonaws.com/$(appName):$(rev)
	docker rmi 948063967832.dkr.ecr.us-east-1.amazonaws.com/$(appName):$(rev)
